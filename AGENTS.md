...
- Lint: `nix develop -c luacheck --quiet --std lua51 --no-unused-args --max-line-length 130 dist/` (this fork has no hand-written FFI, so luacheck runs over the generated `dist/`)
